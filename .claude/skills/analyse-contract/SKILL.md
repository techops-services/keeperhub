...
description: Analyse a smart contract and suggest KeeperHub workflow automations
argument-hint: <contract_address> [chain_id]
---

# Analyse Contract Skill

Analyse a smart contract and suggest KeeperHub workflow automations.

## Usage

```
/analyse-contract <contract_address> [chain_id]
/analyse-contract <abi_json>
```

Examples:
- `/analyse-contract 0x1234...abcd` - Analyse contract on mainnet (chain 1)
- `/analyse-contract 0x1234...abcd 8453` - Analyse contract on Base (chain 8453)
- `/analyse-contract '{"abi": [...]}'` - Analyse from raw ABI

## Instructions

<analyse-contract>

You are a KeeperHub automation expert. Your job is to analyse smart contracts and suggest practical workflow automations that can be built with KeeperHub.
...
### Step 3: Analyse the Contract

Parse the ABI and categorise:
...
**Unmapped Features** (gap tracking):
- Track any functions or events that do NOT map to any KeeperHub capability from Step 1
- Note WHY they can't be automated (missing action type, unsupported pattern, etc.)
- These feed into the Gap Analysis section of the output
...
### Contract Functions Without KeeperHub Coverage

| Function/Event | Type | Why It Cannot Be Automated | What KeeperHub Would Need |
|----------------|------|---------------------------|--------------------------|
| ... | Read/Write/Event | ... | ... |

### Observations
[1-3 bullets about patterns in this contract that KeeperHub should consider supporting]
...
- Always include the Gap Analysis section, even if empty. If every function/event can be automated, state that explicitly with an empty table and a note.
